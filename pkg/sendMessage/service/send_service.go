...
// PollVoteStruct = votar numa enquete existente
type PollVoteStruct struct {
	Number          string   `json:"number"`          // chat onde a enquete está (grupo @g.us ou número)
	PollMessageId   string   `json:"pollMessageId"`   // ID da mensagem de CRIAÇÃO da enquete
	PollCreatorJid  string   `json:"pollCreatorJid"`  // JID de quem criou a enquete (vazio = eu)
	SelectedOptions []string `json:"selectedOptions"` // nomes das opções votadas
}
...
// SendPollVote vota numa enquete existente (usa whatsmeow BuildPollVote).
func (s *sendService) SendPollVote(data *PollVoteStruct, instance *instance_model.Instance) (*MessageSendStruct, error) {
	...
	chatJID, ok := utils.ParseJID(data.Number)
	if !ok {
		return nil, fmt.Errorf("invalid chat jid")
	}

	creatorJID := chatJID
	if data.PollCreatorJid != "" {
		if cj, okc := utils.ParseJID(data.PollCreatorJid); okc {
			creatorJID = cj
		}
	}

	isFromMe := client.Store.ID != nil && creatorJID.User == client.Store.ID.User
	...
	voteMsg, err := client.BuildPollVote(context.Background(), pollInfo, data.SelectedOptions)
	if err != nil {
