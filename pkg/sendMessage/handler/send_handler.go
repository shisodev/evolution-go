...
// @Description Cast a vote on an existing poll (selectedOptions = option names)
// @Tags Send Message
// @Accept json
// @Produce json
// @Success 200 {object} gin.H "success"
...
func (s *sendHandler) SendPollVote(ctx *gin.Context) {
	...
	if len(data.SelectedOptions) == 0 {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "selectedOptions is required"})
		return
	}
